        ...
        for i in range(lnn):
            ...
            # обработка составных графем:
                ...
                if i < lnn-3 and ll==3 and ir(s[0]) and s[1]=="." and ir(s[2]) and grf[i+1].txt=="." and (grf[i+2].tip[0]=="Лек" or grf[i+2].tip[0]=="Lex") and not part:
                    ...
                    grf[i+1].isPart = True
                    grf[i+2].isPart = True
                # ИОФ - И. О. Фамилия - пробел после сокращения имени - три графемы для ФИО
                if i < lnn-3 and ll==1 and ir(s[0]) and grf[i+1].txt=="." and ir(grf[i+2].txt) and len(grf[i+2].txt)==1 and grf[i+3].txt=="." and (grf[i+4].tip[0]=="Лек" or grf[i+4].tip[0]=="Lex") and not part:
                    adC(i,g.txt+grf[i+1].txt+grf[i+2].txt+grf[i+3].txt+" "+grf[i+4].txt,"ИОФ")
                    grf[i+1].isPart = True
                    grf[i+2].isPart = True
                    grf[i+3].isPart = True
                    grf[i+4].isPart = True
